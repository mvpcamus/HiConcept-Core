...
function start(httpConf, dbPath, callback) {
    ...
    require(_homePath+'/lib/router.js')(app); // express router setting
    ...
    if (httpConf.https && typeof httpConf.sslCerts == 'object') {
        ...
        var server = https.createServer(options, app).listen(httpConf.port, function(error) {
            if(error) {
                callback('E211::'+httpConf.port);
            } else {
                require(_homePath+'/lib/addons.js')(app, server, callback); // addon loading
            }
        });
    } else if (!httpConf.https) {
        var http = require('http');
        var server = http.createServer(app).listen(httpConf.port, function (error) {
            if(error) {
