...
module.exports = function(code, callback) {
    ...
    for (var i=1; i<codeArr.length; i++) {
        ...
        if (errors[subCode] !== undefined) {
            errMsg += ((i==1 ? '':' > ') + errors[subCode]);
        } else {
            return errLog('U', errors['X'] + code, callback);
        }
    }
...
// levels: Test > Info > Data > Help > WARN > ERROR > FATAL
