...
function loadAdminInfo(callback) {
    ...
    if (fs.existsSync(initFile)) {
        ...
        if (passed === false || typeof conf !== 'object') {
            ...
            config.adminUser = conf.adminUser;
            config.adminPass = conf.adminPass;
    ...
    // load and decrypt 'initial.json'
        ...
        if (typeof content !== 'object') {
            ...
            config.adminUser = content.adminUser;
            config.adminPass = content.adminPass;
...
function checkNodeJs(callback) {
    var recVer = require(__homePath+'/lib/environment.json').NodejsVer;
